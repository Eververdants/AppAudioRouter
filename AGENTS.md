...
- 外环：设备列表（**多选**），每个设备为一个玻璃胶囊节点
  ...
  - 节点胶囊宽度上限 `max-w-[136px]`（`px-2.5 py-1`），设备名先截断让位给延迟段；延迟段用 max-width 过渡展开（不要 display 切换，否则键盘无法聚焦输入框）
  - 延迟数值段 `max-w-[42px]`、步进段 `max-w-[72px]`（输入框 `w-8`）：这两个数加上内边距不能超过节点胶囊上限，否则展开时会把设备名挤没
