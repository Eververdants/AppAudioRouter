...
- 外环：设备列表（**多选**），每个设备为一个玻璃胶囊节点
  ...
  - **长度上限**：胶囊 `max-w-[132px]`、设备名 `min-w-[40px] max-w-[64px]`（名字是长度的决定项，全名只留在 title 与设置页）。
    典型长度：无延迟约 84px、带延迟值约 126px、展开步进器 132px（= 上限）。
  - 延迟数值段 `max-w-[42px]`、步进段 `max-w-[72px]`（输入框 `w-8`）；「内边距 20 + 名字下限 40 + 步进 72 = 132」必须 ≤ 胶囊上限，
    改任一数值都要重算这三个数，否则展开时会把设备名挤没。延迟段用 max-width 过渡展开（不要 display 切换，否则键盘无法聚焦输入框）
