...
│   │       ├── DelaySegment.tsx        # 设备胶囊尾部的延迟段（有值时显示数值，悬停/聚焦展开 − 数值 +）
...
- 延迟入口在**设备节点胶囊本身**：选中的节点尾部挂 `DelaySegment`（有延迟显示数值，悬停/聚焦展开 −/数值/+ 且数值可键入）；
  不要在舞台底部再做常驻面板，也不要另挂悬浮气泡。`delaySync` 关闭时该段变淡并提示「延迟同步已关闭」；
  设置页仍保留逐设备列表（`DelayStepper`），便于给未选中的设备预设延迟。
...
- 外环：设备列表（**多选**），每个设备为一个玻璃胶囊节点
  ...
  - 选中设备的延迟**并入节点胶囊本身**（`DelaySegment` 挂在胶囊尾部）：有延迟时显示细线分隔 + 数值；悬停或键盘聚焦该节点（`group/device`）时步进器从同一侧滑出，数值段收起——一个设备始终是一个对象，不要另起气泡/面板
  - **长度自适应，不要写死**：胶囊宽度由内容决定，设备名 `min-w-0 truncate` 占它需要的宽度。
    上限有两个，都是推导值而不是像素预算：静息 `MAX_NODE_WIDTH = 2 × (CENTER − ORBIT_RADIUS)`（=132px，轨道水平极端的节点到舞台边缘的余量），
    编辑时 `EDITING_NODE_WIDTH = MAX_NODE_WIDTH + DELAY_STEPPER_WIDTH`（=208px）。
    **展开步进器时上限必须抬升**：否则 132 − 20 内边距 − 76 步进器 = 名字只剩 36px，被挤成 `Hea…`（这就是「延迟占满了」那次事故的成因，已实测复现）。
    上限抬升只对**选中**设备生效（`editingCeiling`），未选中的胶囊悬浮时宽度不应变化。
    **不要再给名字或延迟段设 `max-w-[42px]`/`max-w-[64px]` 这类像素预算**——那是「先有动画目标值再定宽度」，会得到忽长忽短的胶囊。
  - 延迟段的展开/收起用 `grid-cols-[0fr]` ↔ `grid-cols-[1fr]`（`DelaySegment` 里的 `COLLAPSED`/`EXPANDED`）：宽度等于内容自然宽度，
    既能过渡又保持键盘可达（不要 `display` 切换）。数值输入框 `w-9 px-0`——`px-0` 不能省，否则浏览器默认的 2px 内边距会把 `-2500` 裁掉一位。
  - 点击名称按钮选中设备后要 `blur()`（仅指针点击，`e.detail > 0`）：否则按钮保持焦点，`focus-within` 会把步进器一直撑开，
    一屏选中的胶囊全部停在展开态。键盘激活（`detail === 0`）必须保留焦点，步进器才 Tab 得到。
  - 节点外层带 `hover:z-10 focus-within:z-10`：展开的胶囊会变宽，需要盖住相邻节点而不是被它盖住。
