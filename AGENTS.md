...
│   │       ├── DelayReadout.tsx        # 设备胶囊尾部的延迟读数（数值即控件：拖动/滚轮/方向键步进，点击键入）
...
- 延迟入口在**设备节点胶囊本身**：选中的节点尾部是 `DelayReadout`——一个**数值即控件**的读数
  （`名称 │ +180ms`，极细 hairline 竖线分隔）。**没有任何 ± 按钮**：横向拖动按配置步进连续调节
  （4px 一步），滚轮 / 方向键步进（Shift 十倍），点击键入精确值。拖动期间只更新本地预览、
  松手一次性提交（一次手势只留一条日志）；步进与键入即时提交。胶囊宽度因此**恒定**——
  悬停/编辑都不改变任何宽度（历史上那套「悬停展开步进器 + 双宽度上限」的方案已废弃，不要再复活）。
  不要在舞台底部再做常驻面板，也不要另挂悬浮气泡。`delaySync` 关闭时该读数变淡并提示「延迟同步已关闭」；
...
- 外环：设备列表（**多选**），每个设备为一个玻璃胶囊节点
  ...
  - 选中设备的延迟**并入节点胶囊本身**（`DelayReadout` 挂在胶囊尾部）：hairline 竖线 + 签名数值（`+180`，U+2212 负号）+ 小号 `ms`——
    一个设备始终是一个对象，不要另起气泡/面板。数值即控件：拖动/滚轮/方向键/键入（详见「状态管理」的延迟条目），没有 ± 按钮。
  - **长度自适应，但只有一个上限**：胶囊宽度由内容决定，设备名 `min-w-0 truncate` 占它需要的宽度；
    ...
    以 inline style `style={{ maxWidth: MAX_NODE_WIDTH }}` 下发。**悬停/编辑不改变任何宽度**——
    数值控件常驻、不展开，这是「延迟占满了」事故之后定下的铁律：不要复活任何「悬停让胶囊变宽」的设计。
    **不要再给名字或延迟读数设 `max-w-[42px]`/`max-w-[64px]` 这类像素预算**——会得到忽长忽短的胶囊。
  - 数值读数 `min-w-[50px]`：容纳 `−5000ms`，值变化时胶囊宽度不动（避免名字被重新截断的抖动）。
