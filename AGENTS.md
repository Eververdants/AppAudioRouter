...
- 外环：设备列表（**多选**），每个设备为一个玻璃胶囊节点
  ...
  - **长度自适应，不要写死**：胶囊宽度由内容决定，设备名 `min-w-0 truncate` 占它需要的宽度、步进器展开时再把宽度让出来。
    唯一的固定数字是上限 `MAX_NODE_WIDTH = 2 × (CENTER − ORBIT_RADIUS)`（几何推导，=132px：轨道水平极端的节点到舞台边缘的余量），
    超过才截断名字。**不要再给名字或延迟段设 `max-w-[42px]`/`max-w-[64px]` 这类像素预算**——那是「先有动画目标值再定宽度」，会得到忽长忽短的胶囊。
  - 延迟段的展开/收起用 `grid-cols-[0fr]` ↔ `grid-cols-[1fr]`（`DelaySegment` 里的 `COLLAPSED`/`EXPANDED`）：宽度等于内容自然宽度，
    既能过渡又保持键盘可达（不要 `display` 切换）。唯一保留的固定宽度是数值输入框 `w-8`——数字输入框的固有宽度是浏览器默认的约 20 字符。
